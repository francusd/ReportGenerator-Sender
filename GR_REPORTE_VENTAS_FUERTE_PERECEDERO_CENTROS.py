...
ventas_agg=dfventas.groupby(["ALMACEN"]).agg({
       'VENTA HIST':'sum',
       'VENTA ACT' :'sum' 
}).reset_index()
...
resultado=pd.merge(ventas_agg, df_ayer_comparativo,how="inner", left_on='ALMACEN', right_on='ALMACEN')
...
resultado=pd.merge(resultado, plantas,how="inner", left_on='ALMACEN', right_on='Plant_ICG')
resultado= resultado[['Plant','VENTA AYER','VENTA ACT','VENTA HIST','CRECIMIENTO $','CRECIMIENTO %']]
...
df_total.to_excel('C:\DevProjects\DATASETS\\REPORTE_GERENCIAL_VENTAS_FUERTE_PERECEDERO_CENTRO.xlsx', index=True)
