...
result_agg=result.groupby(['ALMACEN','CATEGORIA']).agg({
       'VENTA ACT': 'sum',
       'VENTA HIST':'sum'
}).reset_index()
...
df_ayer_comparativo=df_ventas_ayer.groupby(["ALMACEN",'CATEGORIA']).agg({
       'VENTA ACT': 'sum'
}).reset_index()
...
df_group_comparativo=pd.merge(result_agg, df_ayer_comparativo, left_on=['ALMACEN','CATEGORIA'], right_on=['ALMACEN','CATEGORIA'], how='left')
...
df_group_comparativo= df_group_comparativo[['ALMACEN','CATEGORIA','VENTA AYER' ,'VENTA ACT','VENTA HIST','DIFERENCIA EN VENTAS $','CRECIMIENTO EN VENTAS %']]
...
print(f'DATAFRAME FINAL COLUMNAS \n\n {finale.columns}')

finale= finale[['Plant','CATEGORIA','VENTA AYER' ,'VENTA ACT','VENTA HIST','DIFERENCIA EN VENTAS $','CRECIMIENTO EN VENTAS %','PPTO VENTAS']]
finale=finale.groupby(['CATEGORIA']).agg({
       'VENTA ACT': 'sum',
       'VENTA AYER': 'sum',
       'VENTA HIST': 'sum',
       'DIFERENCIA EN VENTAS $': 'sum',
       'CRECIMIENTO EN VENTAS %': 'sum',
       'PPTO VENTAS':'mean'
}).reset_index()
...
finale= finale[['CATEGORIA','VENTA AYER' ,'VENTA ACT','VENTA HIST','DIFERENCIA EN VENTAS $','CRECIMIENTO EN VENTAS %','PPTO VENTAS','DIFERENCIA vs PPTO $','CUMPLIMIENTO %']]
#finale.to_excel('C:\DevProjects\DATASETS\REPORTE_GERENCIAL_VENTAS_FUERTE_raw.xlsx',index=False)

#----------------------------------------------------------------------------------
# GENERA EL TOTAL DE CADA COLUMNA
...
#print(f'Ventas comparativa (Actual e Historia) \n\n{df_total}')
df_total.to_excel('C:\DevProjects\DATASETS\REPORTE_GERENCIAL_VENTAS_FUERTE_CATEGORIAS.xlsx',index=False)
