...
df_group_comparativo=result.groupby(["SECCION"]).agg({
      'VENTA_HIST':'sum',
       'VENTA ACT': 'sum',
       'COSTO ACT':'sum',
       'COSTO HIST':'sum'
...
result2= result2[['SECCION','VENTA AYER' ,'VENTA ACT','COSTO ACT','VENTA_HIST','CRECIMIENTO','CRECIMIENTO %','BENEFICIO_ACTUAL','MARGEN_ACT_%','BENEFICIO_HISTO','MARGEN_HIST_%']]
...
result2['COSTO ACT']=result2['COSTO ACT'].round(0)
...
total_row['COSTO ACT']=total_row['COSTO ACT'].round(0)
...
columns_to_format = ['VENTA AYER',  'VENTA ACT' ,'COSTO ACT','VENTA_HIST','CRECIMIENTO','CRECIMIENTO %','BENEFICIO_ACTUAL','MARGEN_ACT_%','BENEFICIO_HISTO', 'MARGEN_HIST_%']
df_total[columns_to_format] = df_total[columns_to_format].map(lambda x: "{:,}".format(x))

columns_to_filter = ['SECCION','VENTA AYER',  'VENTA ACT' ,'COSTO ACT','VENTA_HIST','CRECIMIENTO','CRECIMIENTO %','BENEFICIO_ACTUAL','MARGEN_ACT_%','BENEFICIO_HISTO', 'MARGEN_HIST_%']
df_total=df_total[columns_to_filter]

#----------------------------------------------------------------------------------
df_total.to_excel('C:\DevProjects\DATASETS\REPORTE_GERENCIAL_VENTAS_FARMACIA_SUBCATEGORIA.xlsx',index=False)
