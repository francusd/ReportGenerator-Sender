...
df_perecedero=pd.read_excel('C:\DevProjects\DATASETS\REPORTE_GERENCIAL_VENTAS_FUERTE_PERECEDERO_CENTRO.xlsx')
df_perecedero = df_perecedero.loc[:, ~df_perecedero.columns.str.contains('^Unnamed')]

df_perecedero_sub=pd.read_excel('C:\DevProjects\DATASETS\REPORTE_GERENCIAL_VENTAS_FUERTE_PERECEDERO_SUBCATEGORIA.xlsx')
df_perecedero_sub = df_perecedero_sub.loc[:, ~df_perecedero_sub.columns.str.contains('^Unnamed')]


html_table = df_perecedero.to_html(index=False, border=3)
html_table_sub=df_perecedero_sub.to_html(index=False, border=3)
...
color:#004080;">REPORTE GERENCIAL DIARIO DE VENTAS FUERTE PERECEDERO</h5>
<p style="font-family:Arial; font-size:10px;">
Buen día a todos,<br>
Ventas Diarias FUERTE <strong>CENTRO, CATEGORIA</strong> acumuladas al corte: 
<strong>{formatted_yesterday}</strong><br><br>
</p>
"""

# Create both tables
html_centro = build_html_table(df_perecedero, "Ventas FUERTE PERECEDERO - CENTROS")
html_depa = build_html_table(df_perecedero_sub, "Ventas FUERTE PERECEDERO - SUBCATEGORIA")
